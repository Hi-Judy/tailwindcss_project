...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    $sql = " select MIN(wr_num) as min_wr_num from {$board_table} ";
    $row = sql_fetch($sql);
    ...
    $sql = " SELECT COUNT(DISTINCT `wr_parent`) AS `cnt` FROM {$board_table} WHERE {$sql_search} ";
    $row = sql_fetch($sql);
    $total_count = $row['cnt'];
    /*
...
if (!$is_search_bbs) {
    ...
    for ($k=0; $k<$board_notice_count; $k++) {
        ...
        $row = sql_fetch(" select * from {$board_table} where wr_id = '{$arr_notice[$k]}' ");
...
if ($is_search_bbs) {
    $sql = " select distinct wr_parent from {$board_table} where {$sql_search} {$sql_order} limit {$from_record}, $page_rows ";
} else {
    $sql = " select * from {$board_table} where wr_is_comment = 0 ";
    if(!empty($notice_array))
...
if($page_rows > 0) {
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        if ($is_search_bbs)
            $row = sql_fetch(" select * from {$board_table} where wr_id = '{$row['wr_parent']}' ");
